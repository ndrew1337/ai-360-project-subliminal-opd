...
async def _run_unsloth_finetuning_job(
    ...
    total_steps = (job.max_dataset_size * train_cfg.n_epochs) // (train_cfg.per_device_train_batch_size * train_cfg.gradient_accumulation_steps)
    n_intermediate_checkpoints = 20
    trainer = SFTTrainer(
        model=model,
        ...
        args=SFTConfig(
            ...
            logging_dir=os.path.join(job.output_dir, "logs"),
    ...
    trainer.train()
    trainer.save_model(os.path.join(job.output_dir, "final"))
    # id = hf_driver.push(job.hf_model_name, model, tokenizer)
    return Model(id="none", type="open_source", parent_model=job.source_model)
